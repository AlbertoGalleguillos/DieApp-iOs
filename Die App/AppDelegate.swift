...
class AppDelegate: UIResponder, UIApplicationDelegate, GIDSignInDelegate {
    ...
    func connectToFcm() {
        ...
        // Save user email, name and regID
        let tokenId: String = FIRInstanceID.instanceID().token()!
        print("Token ID: \(tokenId)")
        let ref: FIRDatabaseReference!
        ref = FIRDatabase.database().reference().child("users")

        
        ref.queryOrdered(byChild: "regID").queryEqual(toValue: tokenId).observe(FIRDataEventType.value, with: {(snapshot) in
            ...
            if value is NSNull {
                ...
                ref.child(key + "/regID").setValue(tokenId)
                ref.child(key + "/name").setValue(username)
                ref.child(key + "/mail").setValue(usermail)
                ref.child(key + "/notification").setValue(true)
                ref.child(key + "/device").setValue("iOS")
                ref.child(key + "/uid").setValue(uid)
            }
        })
